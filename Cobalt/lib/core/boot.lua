...
if gpu then
  ...
  gpu.set(1, 5, "█████ ██████ ███ ██ ██  ██ ███ ██ ████████ ███████")
  gpu.set(1, 6, "█████     ██     ██    ███ ███ ██     ████ ███████")
  gpu.set(1, 8, "█████              (running on top of OpenOS)█████")
end

-- Report boot progress if possible.
local y = logoScreenHeight + 1
...
local function status(msg)
  if gpu then
    gpu.set(1, y, msg)
    if y == h then
      gpu.copy(1, logoScreenHeight + 2, w, h - 1, 0, -1)
      gpu.fill(1, h, w, 1, " ")
    else
      y = y + 1
    end
...
status("Checking OS type...")

_G.runtype = dofile("/etc/cobalt/runtype.lua")
if runtype == 1 then
  gpu.set(6, 8, "                EMBEDDED                ")
